...
lm_plot <- ggplot(do, aes(x=EDEQ_restraint, y=OE_frequency)) + 
  geom_point() +
  ggtitle("Over-eating vs Dietary restraint") +
...
> We could think of this relationship, therefore, as $BMI = slope*EDEQrestraint + intercept$ ... In fact, that's how we described this in EDUC 641, .purple[but that's not quite right]
...
.blue[*What is wrong about describing the relationship between over-eating frequency and eating restraint as we did on the previous slide?*]
...
.blue[All of the models we will focus on in this course are linear. Why?]
...
.red[**In fact, linear modeling is so tractable that this is what we will spend the entire course on!**]
...
$$OvereatingFrequency = \beta_0 + \beta_1(DietaryRestraint)$$
---
...
```{r, echo = F}
do %>% 
  select(EDEQ_restraint, OE_frequency) %>% 
  tail()
```
...
$$OvereatingFrequency = \color{orange}{(\beta_0)} + \color{purple}{(\beta_1)}(DietaryRestraint)$$
...
$$OvereatingFrequency = \color{orange}{20} + \color{purple}{1.5}(DietaryRestraint)$$
**What is the expected over-eating frequency for a Dietary Restraint rating of 2?**
...
$$OvereatingFrequency = \color{orange}{20} + \color{purple}{1.5}(2) = 20 + 3 = 23$$
**For a Dietary Restraint rating of 2, the predicted over-eating frequency value is 23.**
...
Is Dietary Restraint a perfect predictor of Overeating Frequency? 
...
$$Overeating = \beta_0 + \beta_1(\text{Dietary Restraint}) + \beta_2(\text{Meal Frequency}) + \beta_3(\text{Nutritional Habits})... \beta_\infty$$
...
In a regression model, all the variability that we couldn't explain with our predictor is condensed into a .green[residual term] $\color{green}{\varepsilon}$.
...
do %>% 
  sample_n(100) %>% 
  ggplot(aes(x=EDEQ_restraint, y=OE_frequency)) + 
  geom_point() + 
...
$$OvereatingFrequency = \beta_0 + \beta_1(DietaryRestraint) + \color{green}{\varepsilon}$$
---
...
```{r, echo = F, comment = NA}
resid_table <- do %>% 
  select(OE_frequency, predict) %>% 
  rename(predicted_OE = predict) %>% 
  mutate(residual = OE_frequency - predicted_OE,
...
```{r, echo = F}
good_fit <- do %>% 
  mutate(predict = (1 + 0.85*EDEQ_restraint))
set.seed(100)
...
```{r, echo = F}
bad_fit <- do %>% 
  mutate(predict = (3 + 1.5*EDEQ_restraint))
set.seed(100)
...
bad_fit_plot
```
$\Sigma(e_i)^2 = 22971.23$
]

Using the OLS method, the regression line on the left is a better fit (13,691 < 22,971).
...
$$SSR_\text{OEfreq} = \sum{(Y_i - \bar{Y})^2}$$
...
do %>% 
  ...
  ggplot(aes(x = participant_id, y = OE_frequency)) +
  geom_point() +
  geom_hline(yintercept = mean(do$OE_frequency), color = "blue") +
  geom_segment(aes(xend = participant_id, yend=mean(do$OE_frequency)), col = "green", alpha = 0.5) +
  geom_label(x = 15, y = mean(do$OE_frequency), label = "Mean Overeating Freq.", color= "blue")
...
We can partition the outcome's total variance $(SS_{OEfreq})$ into:
* Model-accounted variance $(SS_\text{Model})$
* Residual variance $(SS_\text{Residual})$

$$SS_\text{OEfreq} = SS_\text{Model} + SS_\text{Residual}$$
--
...
---
# Partitioning variance
$$\text{If } SS_\text{OEfreq} = \color{purple}{SS_\text{Model}} + {SS_\text{Residual}}$$
...
do %>% 
  ...
  geom_segment(aes(xend = participant_id, y = mean(do$OE_frequency), yend=predict), col = "purple", alpha = 0.5) +
  geom_hline(yintercept = mean(do$OE_frequency), color = "blue")
...
---
# Partitioning variance
$$\text{If } SS_\text{OEfreq} = \color{purple}{SS_\text{Model}} + \color{green}{SS_\text{Residual}}$$
...
$$R^2 = \frac{SS_\text{Model}}{SS_\text{OEfreq}}$$
--
