...
car::Anova(m1, type = 3)
```
Here we can see all the information we just calculated earlier. Only the p-value is new!
...
```{r, echo = F, eval = F}
my_df <- iris
petal_lm <- lm(Petal.Length ~ 0 + Sepal.Length, data = my_df)
...
#Sample points
petal_lm_surface <- my_df$Sepal.Length
...
fig <- iris_plot %>% 
  ...
  add_surface(z = petal_lm_surface,
              ...
              opacity = 0.5)
fig
fig %>% animation_opts(1000, transition = 500)
library(plotly)
...
needs(dash)
needs(dashCoreComponents)
needs(dashHtmlComponents)
